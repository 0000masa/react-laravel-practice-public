...
### 9-7. ドキュメント整合性（このリポジトリ自体の改善）

スキルシート掲載リポジトリとして、**README/構成図と実コードの一致は信頼性に直結**します。本対策と合わせて以下を修正済み:

- README の「API Gateway → VPC Link」→ 実態の「CloudFront `/api/*` → ALB（秘密ヘッダーで保護）」に修正。
- README の「RDS MySQL 8.0」→ 実態の「RDS MariaDB 11.4」に修正。
- コスト試算の「API は ALB 直接 / WAF は frontend のみ」→ 実態の「API も CloudFront+WAF 経由」に修正。
- `environment_setup.md` の「本番で静的アクセスキー設定」→ 実態の「ECS タスクロール利用」に修正。

> このこと自体を面接で「公開前にコードと突き合わせて記述の不整合を 4 件発見し修正した」と前向きに語れます。
...
### Q. 「構成図と実装が違うのでは？」（最重要・自分から触れてもよい）

ご指摘の通り、当初 README に API Gateway 経由と書いていましたが、実装は CloudFront の `/api/*` ビヘイビアで ALB に流す構成です。公開前にコードと全ドキュメントを突き合わせ、API 経路・RDS エンジン・WAF 適用範囲・認証方式の **記述不整合 4 件を発見して修正**しました。設計判断としては、フロントと API を同一ドメインで配信して CORS/Cookie を簡素化し、WAF を 1 箇所（CloudFront）で全リクエストにかけたかったため ALB 直結 + 秘密ヘッダー保護を選んでいます。
...
- [github_actions_secrets.md](./github_actions_secrets.md) — OIDC sub/ref・ForAllValues の罠
- [iam_passrole_for_ecs.md](./iam_passrole_for_ecs.md) — PassRole 二重ロック
- [module-refactoring.md](./module-refactoring.md) — 単一モジュール化・moved ブロック
- [ecs-config-variables.md](./ecs-config-variables.md) — capacity provider 変数化・bake time
- [ecspresso-deployment-pipeline.md](./ecspresso-deployment-pipeline.md) — B/G デプロイと必要権限
- [codedeploy_ecs_deployment.md](./codedeploy_ecs_deployment.md) — CodeDeploy 方式との比較
- [db-task-workflow.md](./db-task-workflow.md) — 承認付き DB 運用タスク
- [sqs_queue_qrcode.md](./sqs_queue_qrcode.md) / [batch_daily_report.md](./batch_daily_report.md) — 非同期/バッチ
- [aws-cost-estimation-verified.md](./aws-cost-estimation-verified.md) — コスト試算
</content>
</invoke>
