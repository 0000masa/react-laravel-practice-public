...
- **料金取得日**: **2026-05-11**（前回 2026-04-21 から更新）。**RDS 節のみ 2026-07-07 に単価を再検証・更新**（RDS 節と末尾の差分メモ参照）
...
- **東京リージョン単価**: db.t4g.micro Single-AZ **$0.025/時間**、gp3 **$0.138/GB-月**（2026-07-07 取得。料金ページが価格表の描画に使う公開 JSON＝料金 API の 2026-07-03 公開データより）

#### 月額計算（全シナリオ共通、autoscale 影響なし）

| 項目 | 計算 | 月額 |
|---|---|---|
| db.t4g.micro インスタンス時間 | `0.025 × 730` | **$18.25** |
...
>
> Aurora に乗せ替えた場合との比較・損益分岐は [aws-rds-selection-applied.md](./aws-rds-selection-applied.md) を参照。
...
|---|---|---|
| **低**（1–40 RPS） | **≈ $137** | ≈ $176 |
| **中**（40–150 RPS） | **≈ $202** | ≈ $289 |
| **高**（150–300 RPS） | **≈ $612** | ≈ $792 |

### 低トラフィック内訳（Spot ベース）

| サービス | 月額 (USD) | 比率 |
|---|---|---|
| NAT Gateway | $47.12 | 34.5% |
| ALB（IPv4 含む） | $34.16 | 25.0% |
| RDS | $21.01 | 15.4% |
| ECS Fargate Spot | $16.89 | 12.4% |
| CloudWatch（Container Insights enhanced 含む） | $9.41 | 6.9% |
| WAF | $6.60 | 4.8% |
| Route 53 | $0.55 | 0.4% |
| S3 | $0.43 | 0.3% |
| CloudFront | $0.30 | 0.2% |
...
| **合計（Spot）** | **≈ $136.57** | 100% |
| 参考: OnDemand 換算合計 | ≈ $175.91 | – |

### 中トラフィック内訳（Spot ベース）

| サービス | 月額 (USD) | 比率 |
|---|---|---|
| ALB（IPv4 含む） | $45.84 | 22.7% |
| ECS Fargate Spot（平均 2.5 task） | $37.13 | 18.4% |
| CloudWatch | $36.24 | 17.9% |
| NAT Gateway | $51.46 | 25.5% |
| RDS | $21.01 | 10.4% |
| WAF | $7.80 | 3.9% |
| CloudFront | $0.90 | 0.4% |
| その他（S3 / Route 53 / SES / SQS 等） | $1.78 | 0.9% |
| **合計（Spot）** | **≈ $202.16** | 100% |
| 参考: OnDemand 換算合計 | ≈ $288.73 | – |

### 高トラフィック内訳（Spot ベース）

| サービス | 月額 (USD) | 比率 |
|---|---|---|
| CloudFront（無料枠超過）| $226.54 | 37.0% |
| CloudWatch | $108.05 | 17.7% |
| ALB（IPv4 + LCU 増） | $86.72 | 14.2% |
| ECS Fargate Spot（6 task） | $84.37 | 13.8% |
| NAT Gateway | $63.86 | 10.4% |
| RDS | $21.01 | 3.4% |
| WAF | $15.00 | 2.5% |
| SES / SQS / X-Ray / S3 等 | $5.73 | 0.9% |
| Route 53 | $0.60 | 0.1% |
| **合計（Spot）** | **≈ $611.88** | 100% |
| 参考: OnDemand 換算合計 | ≈ $792 | – |
...
>
> ※ この表の「今回」列は 2026-05-11 時点の値。低トラフィック合計はその後の RDS 再検証（下記）で $137（Spot）に更新済み。

## 2026-07-07 差分（RDS 単価の再検証）

- db.t4g.micro Single-AZ: **$0.017 → $0.025/時間**。RDS 小計 **$15.17 → $21.01**、各シナリオ合計に **+$5.84**。gp3 単価（$0.138/GB-月）は変更なし。
- 旧値 $0.017 が値上げ前の実勢だったのか取得時の誤りだったのかは判別できない。今回は料金ページが価格表の描画に使う公開 JSON（料金 API の 2026-07-03 公開データ）から取得しており、取得手順の再現方法は [aws-rds-selection-applied.md](./aws-rds-selection-applied.md) の単価表と同一。
- あわせて高トラフィックのサマリ値を内訳合計（$606 → 更新後 $612）と整合するよう修正（従来はサマリ ≈$590 と内訳 ≈$606 が不一致だった）。
