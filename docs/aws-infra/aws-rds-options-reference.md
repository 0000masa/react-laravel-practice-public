...
flowchart TD
    ...
    D -->|読み取りを大きく伸ばしたい| E{RDS のレプリカで足りる？}
    E -->|足りる / エンジンを変えたくない| RDSRR[RDS リードレプリカ §3.4<br/>最大15台・非同期・手動増減]
    E -->|遅延を小さくしたい<br/>台数を自動増減したい| AUR[Aurora プロビジョンド<br/>+ リードレプリカ Auto Scaling]
...
>
> **読み取りスケールは「まず RDS レプリカ」から**: 読み取りが重いからといって即 Aurora ではありません。**リードレプリカは Aurora 専用の機能ではなく RDS 標準の全エンジンで使える**ため（§3.4）、エンジンを変えずに現行構成のまま台数を足せます。Aurora へ跳ぶと（MariaDB からなら）エンジン移行が伴い、同一クラスの単価も上がります。損得の実数は [`aws-rds-selection-applied.md`](./aws-rds-selection-applied.md) を参照。
>
> **横串の軸（スケールとは独立）**: Multi-AZ（インスタンス構成・§3.2）はこのフローの分岐には現れません。これは「何をスケールさせるか」ではなく**可用性のオプション**で、上記のどの RDS 構成にも上乗せできる**直交した軸**だからです。読み取り/書き込みのスケールには寄与せず（スタンバイは読めない＝冗長専用）、**可用性要件（RPO/RTO・許容ダウンタイム・SLA）で ON/OFF を判断**します。なお「Aurora は構成上もともと 3 AZ 冗長」というのは**ストレージ層の話**で、コンピュート層の AZ 冗長は「別 AZ にリーダーを置く」ことで別途確保します（§3.5）。
>
...
各 DB インスタンスは自分専用の EBS ボリュームを持ちます。プライマリに追加する台は 2 種類あり、**Multi-AZ スタンバイ**は同期複製で**読み取りには使えない冗長専用**、**リードレプリカ**は非同期複製で**読み取りに使える**という違いがあります。どちらも **別ボリュームにデータを丸ごとコピー**するため、台数を足すたびにストレージ分のコストがまるごと増えます。また非同期であるリードレプリカでは、コピーである以上レプリケーション遅延が発生します。
...
flowchart LR
    subgraph RDS["RDS（標準）: ローカルストレージ型"]
        P1["プライマリ / ソース<br/>読み書き"] --- S1[(EBS A)]
        ST["スタンバイ<br/>読めない（冗長専用）"] --- S2[(EBS B)]
        RR["リードレプリカ<br/>読める"] --- S3[(EBS C)]
        P1 -. 同期複製 .-> ST
        P1 -. 非同期複製（binlog/WAL）<br/>遅延あり .-> RR
    end
...
> **ここから派生する重要な帰結**: ストレージ層が独立しているため、Aurora で選ぶのは実質「**コンピュート層をどう買うか**」だけです。**プロビジョンド（固定インスタンスクラス）と Serverless v2（ACU レンジ）は別サービスではなく、同じ Aurora クラスターの中で「インスタンスごとに」選ぶ設定**で、ライターは固定クラス・リーダーは Serverless v2、といった混在も可能です。共有分散ストレージ・可用性（約 30 秒でフェイルオーバー）・リーダー最大 15 台は**両者でまったく同じ**で、違うのは容量の指定方法（クラス or ACU レンジ）と課金（インスタンス時間 or ACU×秒）だけ。→ [§3.5](#35-auroraプロビジョンド) / [§3.6](#36-aurora-serverless-v2)
...
- **可用性**: ◎。リーダーが**フェイルオーバー先を兼ねる**。ライター障害時は**レプリカ遅延の少ないリーダー**が新しいライターに昇格し、通常 35 秒程度で切替
- **ストレージ・アーキ**: ローカル。**3 台がそれぞれ自分のストレージを持つ**（Aurora のような共有ボリュームではない → 下の注記）
...
> **Aurora と混同しやすい最大のポイント**: 「1 ライター + 2 リーダー」という構成が Aurora そっくりですが、**ストレージは共有していません**。AWS 公式も「Multi-AZ DB clusters aren't the same as Aurora DB clusters」と明記しており、複製は**エンジンのネイティブ機能**でライターから各リーダーへ**データを送る**方式です。だからこそ**レプリカ遅延が存在します**（共有ボリュームなら原理的に発生しません）。対応インスタンスクラスが `d` サフィックス（ローカル NVMe 内蔵）系に限定されるのも、各インスタンスが自前の高速ストレージを必要とするからです。
>
> **「共有ストレージが落ちたら全滅では？」という心配は不要**: そもそも共有していないため、**3 AZ に独立した 3 つのストレージ**があります。ライターの AZ がまるごと落ちても残り 2 台のストレージは無事で、遅れの少ないリーダーが昇格して復旧します。単一のストレージが単一障害点になる構造ではありません。

> **Multi-AZ インスタンス vs Multi-AZ DB クラスターの一言整理**: 前者はスタンバイ 1 台で**読めない**（冗長専用）。後者はリーダー 2 台で**読める**（冗長 + 読み取り分散）。

### 3.4 RDS リードレプリカ

ソース DB から非同期レプリケーションする読み取り専用インスタンス。読み取りスケールアウトの基本手段で、**Aurora 専用の機能ではなく RDS 標準の全エンジン（MariaDB / MySQL / PostgreSQL / Oracle / SQL Server / Db2）で使えます**。
...
- **注意**: アプリ／プロキシ側で「書き込みはソース、読み取りはレプリカ」と**接続先を分ける設定**が必要（自前で SQL を解析するコードを書く話ではない → [補足](#補足-読み書きの振り分けは誰がやるか)）。遅延に耐えられない読み取り（書いた直後に必ず読む等）はソースへ。
...
| **エンドポイント** | クラスター / リーダーエンドポイントが自動振り分け | レプリカごとに個別。**アプリ／プロキシ側で接続先を分ける設定が必要**（→ [補足](#補足-読み書きの振り分けは誰がやるか)） |
...
「アプリ側で振り分け」と書くと SQL を解析するコードを自作するように読めますが、実体は**接続先を分ける設定**です。RDS のレプリカには Aurora のリーダーエンドポイントに相当する「1 つ叩けば AWS が振り分けてくれる名前」が無く、`mydb-replica-1.xxxx.rds.amazonaws.com` のような**レプリカ固有のエンドポイント**しか生えないため、宛先を選ぶ主体が DB の外側に必要になる —— これが話の出発点です。選択肢は 3 層あります。

| 層 | 実現方法 | 備考 |
|---|---|---|
| **フレームワーク** | Laravel の `read` / `write` 接続定義（+ `sticky`）、Rails の `connects_to` など | 専用ライブラリ不要。本プロジェクト（Laravel）ならここ |
| **ドライバ** | MySQL Connector/J の replication ドライバ、AWS Advanced JDBC Driver など | 主に Java 系。PHP / PDO には相当する標準機能なし |
| **プロキシ** | ProxySQL、MariaDB MaxScale（`readwritesplit` ルーター） | SQL を見て自動振り分け。アプリは無改修だがプロキシ自体の運用・冗長化が増える |

> **RDS Proxy はこの用途ではありません。** 役割は接続プーリングとフェイルオーバー短縮（§4.5）で、SQL を見て読み書きを自動分離する機能は持ちません。

Laravel なら `config/database.php` の接続定義に `read` / `write` を足すだけで、`SELECT` は `read`、更新系と**トランザクション内の全クエリ**は `write` へ自動で振り分けられます。`read.host` に複数のホストを書いた場合は**接続確立時にランダムで 1 台**選ばれる（＝負荷分散の実体はこれだけ）。

**設定より難しいのは次の 2 点**で、ここが「実装が必要」と言われる中身です。

1. **どの読み取りをレプリカに逃がしてよいかの選別**: 非同期複製なので数秒遅れうる。`sticky => true`（同一リクエスト内で一度書き込んだら以降の読み取りもライターへ）は有効だが、**リクエストをまたぐケース**（POST → リダイレクト → GET）は守られないため、個別にライターを指定する（Laravel なら `->useWritePdo()`）判断が要る。
2. **台数の増減がアプリのリリース作業になる**: ホスト名を列挙する以上、レプリカを足し引きしたら設定変更＋デプロイが必要。Aurora のリーダーエンドポイント + レプリカ Auto Scaling（§4.1）ならここが不要になる、というのが下の比較表の差の実務的な意味。

#### Multi-AZ インスタンス（3.2）との併用

**併用できます。** AWS 公式にも「Multi-AZ 構成でスタンバイを持つ DB インスタンスに対してリードレプリカを構成できる」と明記されています。目的が直交しているためで、**スタンバイは可用性のため（読めない）／レプリカは読み取りスケールのため（HA ではない）**—— 片方だけでは片方の要件を満たせません（§0 の「横串の軸」参照）。

```
        ┌── 同期複製 ───→ スタンバイ（AZ-c）        読めない・冗長専用
プライマリ（AZ-a）
        ├── 非同期複製 ─→ リードレプリカ 1（AZ-b）  読める
        └── 非同期複製 ─→ リードレプリカ 2（AZ-c）  読める … 最大 15 台
```

- **スタンバイは常に 1 台**。レプリカを 15 台まで増やしても、Multi-AZ の同期ペアは**プライマリ + スタンバイ 1 台**の 1 組だけです（それが Multi-AZ インスタンス構成の定義）。
- ただし**各レプリカを個別に Multi-AZ にすることは可能**で、その場合はそのレプリカ専用のスタンバイが別途増えます。これは**ソース側の Multi-AZ 設定とは独立**に選べます。
- **コストの数え方**: プライマリ + スタンバイ（実質 2 倍）+ レプリカ台数分。いずれもストレージ込みで課金されます。

#### RDS リードレプリカ（3.4）vs Aurora リーダー（3.5）

**名前が似ているだけで中身は別物**です。RDS はデータを複製し、Aurora は複製せず同じボリュームを見ます（§2 の図がこの違いを描いています）。

| 観点 | RDS リードレプリカ | Aurora リーダー |
|---|---|---|
| **複製の仕組み** | エンジンのネイティブ複製（binlog/WAL）を**非同期で流す** | **複製しない**。同じクラスターボリュームを見るだけ |
| **ストレージ** | レプリカごとに**別ボリューム**（データを丸ごとコピー） | **共有**。台数を増やしても増えない |
| **レプリケーション遅延** | 秒オーダーになりうる | 通常 100ms 未満 |
| **追加コスト** | インスタンス + **ストレージまるごと** | インスタンスのみ |
| **台数の増減** | 手動（最大 15）。ソースのスナップショットから作るため追加に時間がかかる | Auto Scaling 可（最大 15）。ボリューム共有なので追加・削除が速い |
| **昇格** | 手動昇格（スタンドアロン化。HA 用ではない） | **自動フェイルオーバー先**（約 30 秒で昇格） |
| **接続の振り分け** | レプリカごとに個別。アプリ／プロキシ側の設定が必要 | **リーダーエンドポイント**が自動で負荷分散 |
...
> **Aurora の「Multi-AZ」はストレージではなくコンピュートの話**: クラスターボリュームは、**インスタンスがどの AZ にいるかと無関係に**常に 3 AZ・6 重コピーで保持されます（AWS 公式: "regardless of whether the instances in the DB cluster span multiple Availability Zones"）。したがって **RDS のような「Multi-AZ を ON にする」設定は存在しません** —— 常時 ON で外せないからです。
>
> 一方、**Aurora で「Multi-AZ 配置にする」＝「別 AZ にリーダーを置く」**（コンピュート層の冗長化）であり、これは設計者が選ぶことです。別 AZ にリーダーがあれば AZ 障害時にそれを昇格して**通常 30 秒未満**で復旧しますが、**単一インスタンス（またはリーダーが全て同じ AZ）だと、データは無事でも新しいプライマリを作り直すため最大 10 分程度書き込みができません**。「Aurora は最初から 3 AZ 冗長」はストレージの話であって、リーダーを置かなくてよいという意味ではありません。
...
- RDS Multi-AZ DB インスタンス（同期複製・スタンバイは読み取り不可）: <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.MultiAZSingleStandby.html>
...
- RDS リードレプリカ 全般（全エンジン対応・Multi-AZ との併用）: <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_ReadRepl.html>
...
- Aurora の高可用性（ストレージは常に多 AZ・リーダー昇格・単一インスタンス時の注意）: <https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.AuroraHighAvailability.html>
