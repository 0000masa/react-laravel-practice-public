...
### Google ドキュメントに貼り付ける場合

この資料は表が多い。Google ドキュメントにそのまま貼ると、設定によっては表がパイプ記号（`|`）の生テキストのまま貼り付いてしまう。先に次の設定を有効にしておくこと。

> Google ドキュメント → メニューの **ツール** → **設定** → **「Markdown を自動検出」にチェック**

これを有効にした状態で本文を貼り付けると、見出し・表・箇条書きがすべて Google ドキュメントの書式に変換される。
